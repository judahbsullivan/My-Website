  ...
  <section class="space-y-4">
    ...
    <UiBentoBox ref="sectionBoxRef" container backdrop border shadow padding="lg" rounded="2xl"
      ...
      <div ref="gridRef" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        <NuxtLink v-for="(project, index) in projects" :key="project.id" :to="`/projects/${project.slug}`"
          class="group block">
...
onMounted(() => {
  registerExitAnimation('projectsFeatured', animateExit)
  nextTick(() => {
...
onUnmounted(() => {
  unregisterExitAnimation('projectsFeatured')
