  ...
  <section class="space-y-4">
    ...
    >
      ...
      >
        ...
        >
          <div class="w-full">
            <div v-if="playground.previewImageUrl" class="mb-3 rounded-lg overflow-hidden aspect-video bg-muted relative">
              <img
            ...
            <div v-else class="mb-3 rounded-lg aspect-video bg-muted flex items-center justify-center relative">
              <Icon name="heroicons:code-bracket" class="w-10 h-10 text-muted-foreground" />
              <div class="absolute top-3 right-3 flex gap-2">
                <span :class="[
                  'px-2 py-1 text-xs font-medium rounded-full',
                  playground.isVueComponent ? 'bg-primary/20 text-primary' : 'bg-secondary/20 text-blue-500'
...
onMounted(() => {
  registerExitAnimation('playgroundGrid', animateExit)
  nextTick(() => {
...
onUnmounted(() => {
  unregisterExitAnimation('playgroundGrid')
