<template>
  <section class="space-y-4">
    <div ref="titleWrapperRef" class="overflow-hidden">
      <UiTitle ref="titleRef" as="h2" size="lg" weight="bold" class="text-2xl sm:text-3xl font-bold text-foreground">
        Featured
      </UiTitle>
    </div>

    <UiBentoBox ref="sectionBoxRef" container backdrop border shadow padding="lg" rounded="2xl"
      ...
      <div ref="gridRef" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        <NuxtLink v-for="(playground, index) in playgrounds" :key="playground.id" :to="`/playground/${playground.slug}`"
          class="group block">
          <UiBentoBox :ref="el => setCardRef(el, index)" :size="'lg'" :variant="getVariant(index)" :hover="true"
            :className="'flex-col items-start justify-between h-full opacity-0 scale-90 -rotate-3'">
            <div class="w-full">
              <div v-if="playground.previewImageUrl"
                class="mb-4 rounded-lg overflow-hidden aspect-video bg-muted relative">
                <NuxtImg 
                ...
                <div class="absolute top-3 right-3 flex gap-2">
                  ...
                  <span v-if="playground.requiresSubscription"
                    class="px-2 py-1 text-xs font-medium rounded-full bg-amber-500/20 text-amber-500 flex items-center gap-1">
              ...
              <div class="flex items-center gap-2 mb-3">
                ...
                <span v-if="playground.isFeatured"
                  class="px-2 py-1 text-xs font-medium rounded-full bg-amber-500/20 text-amber-500">
                  Featured
                </span>
              </div>
              <h3
...
function setupScrollAnimation() {
  ...
  import('gsap/ScrollTrigger').then((stModule) => {
    ...
    nextTick(() => {
      ...
      gsap.set(box, { opacity: 0, scale: 0.95, y: 24 })
      if (cards.length > 0) gsap.set(cards, { opacity: 0, y: 20, scale: 0.9, rotation: -3 })
